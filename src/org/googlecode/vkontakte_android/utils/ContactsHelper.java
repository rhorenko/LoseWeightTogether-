...
public class ContactsHelper {
	...
	public String getName(long vkId) {
		...
		try {
			...
			if (!q.moveToFirst())
				return null;
			return q.getString(q
					.getColumnIndexOrThrow(Contacts.PeopleColumns.NAME));
	...
	public Uri findPersonByVkId(long vkId) {
		...
		final Cursor q = contentResolver.query(
				...
				Contacts.ContactMethodsColumns.AUX_DATA + "=? and "
						+ Contacts.ContactMethodsColumns.DATA + "=" + vkId
						+ " and " + Contacts.ContactMethodsColumns.KIND + "="
						+ Contacts.KIND_IM,
		...
		try {
			...
			if (!q.moveToFirst())
				return null;
			return createPersonUri(q.getString(q
	...
	public static Uri createPersonUri(String contactId) {
		return Contacts.People.CONTENT_FILTER_URI.buildUpon()
				.appendPath(contactId).build();
	}

	/**
	 * Вычищает vkId из всех контактов в
	 * локлаьной БД
	 * 
	...
	public int deleteEverything() {
		return contentResolver
				.delete(Contacts.ContactMethods.CONTENT_URI,
						Contacts.ContactMethodsColumns.AUX_DATA + "=? and "
	...
	/**
	 * Добавляет человеку vkId и добавляет
	 * контакт в группу
	 * 
	 * @param person
	 *            запись из Contacts
	...
	public Uri bindPersonWithVkontakteId(Uri person, long vkId) {
		...
		vk.put(Contacts.ContactMethodsColumns.TYPE,
		...
		Uri lastInserted = contentResolver.insert(
				Contacts.ContactMethods.CONTENT_URI, vk);

		// int groupId = findOrCreateGroup(getString(R.string.group_name),
		// getString(R.string.group_notes));
	...
	private int findOrCreateGroup(String name, String notes) {
		...
		final Cursor q = cr
				.query(Contacts.Groups.CONTENT_URI, new String[] {
		...
		try {
			if (!q.moveToFirst()) {
				...
				return Integer.parseInt(cr.insert(Contacts.Groups.CONTENT_URI,
						vk).getLastPathSegment());
			}
